...
you never misattribute work or grant the wrong person production access. Last updated 2026-08-31.
...
| **Carlos Obregon** | `Obregon1993` | Carlos.Obregon@rxcs.net | Senior SW Engineer | **Production gatekeeper** (added 2026-08-31) — direct push to eMed/emed_sql `main`, `maintain` role on eMed (deploy tags). **Not** the same person as Carlos Cueto — see gotcha below. |
...
Cardell**, **Carlos Cueto**, **Jose Daniel Garcia Gonzalez** (backup), and **Carlos Obregon**
(added 2026-08-31). See
...
- **Carlos Cueto** = `carcuet` — senior DB engineer, a **production gatekeeper** (esp. prod DB / emed_sql).
- **Carlos Obregon** = `Obregon1993` — senior SW engineer, **also a production gatekeeper** since 2026-08-31.

Both Carloses are now gatekeepers, which makes a bare "Carlos" **more** ambiguous, not less — in any
context (attribution, review, DB work, emails, access changes), **ask which Carlos** or use the
**Carlos 1** (= Carlos Cueto) / **Carlos 2** (= Carlos Obregon) aliases — see Nicknames & aliases
below. Historical note: before 2026-08-31 Obregon was not a gatekeeper, so older docs/notes saying
"Carlos" in a production context mean Cueto.
...
- **"Carlos" alone is ambiguous — always.** Since 2026-08-31 both Carloses are production
  gatekeepers, so a production/DB/merge context no longer disambiguates. **Ask which Carlos** or use
  the Carlos 1 / Carlos 2 aliases. Never guess.
