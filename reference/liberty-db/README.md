...
A strictly **read-only** extraction from the RXCS Liberty SQL Server (`LIBERTYSVRRXCS`, SQL Server 2022 — renamed from `libertyserver.localdomain` on 2026-09-16; the old name no longer resolves), reachable only from the office LAN / RXCS job servers using the `SRC_*` credentials in `emed_etl/.env`. Production flows take the name from the Prefect Secret block `rxcs-database-server`; the hardcopy image share is `\\LIBERTYSVRRXCS\LCS\ImageCTL` (block `rxcs-image-root`). Structure came from the `sys.*` / `INFORMATION_SCHEMA` catalog views; relationships were inferred from column naming and then **data-validated** against real key values (aggregate counts only — no patient rows were ever read). See the Regeneration section at the bottom.
...
This reference was generated by a strictly **read-only** extraction from the RXCS Liberty database (`RXQRXCOMPOUNDSTORE` on `LIBERTYSVRRXCS`, formerly `libertyserver.localdomain`, reachable only from the office LAN) using the `SRC_*` credentials in `emed_etl/.env`, followed by a documentation pass.
