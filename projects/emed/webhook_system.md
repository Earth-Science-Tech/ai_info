...
> **Externally-prescribed orders: `OrderId == VisitId` (verified 2026-08-05).** For an order submitted via the External Prescription API (`POST /emed/order`), the returned **`OrderId` is literally `moct_visit.id`** — the same integer as the MOCT `VisitId`. It is **NOT** `clinic_order_id` (that column is NULL for external orders; when a clinic DOES pass its own `OrderId` on the *MOCT-intake* path `POST /moct/visit`, it lands in `clinic_order_id` and surfaces as **`OrderTag`** — a *different* value, don't conflate). Chain: `create_external_prescription` reads back `visit_id = tmp2[0].id` → `post_emed_order` returns `{ OrderId: visit_id }` → `get_emed_order` does `visit_status(:OrderId, clinic, 0)` (PK lookup). The **webhook `data` block always uses the `VisitId` field** (worker calls `visit_status(visit_id, clinic)` with no 3rd arg → `is_visit=1`), so an external order's delivery carries `data.VisitId` = the clinic's `OrderId`. `GET /emed/order/:OrderId` (is_visit=0) is the *only* surface that renames the field to `OrderId` and drops `Clinic`/`VisitType`; otherwise its body == `GET /moct/visit/:VisitId`. Docs made explicit in `api-docs-content.ejs` (Overview callout + External-Prescription cross-ref + `data`-block note), 2026-08-05.
...
- `server/webhook_worker.js` — `start()` / `stop()` / `kick()` / `invalidate_secret_cache()`. Single in-process worker, conditional UPDATE claim, exponential backoff (60s → 6h capped, 8 attempts), **self-healing cool-down** (pause + auto-retry; hard-disable only after 30d continuous failure) + per-config ~4s delivery pacing — see Operational notes.
...
- **Self-healing cool-down** (prod 1.0.186, 2026-08-05 — replaces the old permanent auto-disable): 20 consecutive `dead` deliveries now **PAUSE** the config (`paused_until = now + 6h`, `is_active` STAYS 1) instead of disabling it. Events keep buffering (the proc/emitter still enqueue for `is_active=1`); when the cool-down expires the worker auto-resumes and the reviver re-drives the backlog; each cool-down sends ONE **paced** probe and any success fully resets it. A config is **HARD-disabled (`is_active=0`, needs a human) ONLY after 30 days of *continuous* failure AND past the pause threshold** — so a low-volume clinic can't be cut off by sparse failures. New `emed_webhook_config` cols: `paused_until` / `failing_since` / `last_send_at`. Per-config **~4s delivery pacing** (one delivery/config/tick, gated on `last_send_at`) so a recovering clinic's buffered backlog flushes paced, never as a burst. **Config-level + event-agnostic** — applies identically to all `moct.*` AND `pharmacy.script.*` events. Env: `WEBHOOK_PAUSE_AFTER_FAILURES` / `WEBHOOK_COOLDOWN_HOURS` / `WEBHOOK_HARD_DISABLE_DAYS` / `WEBHOOK_DELIVERY_PACE_SECONDS`. Migration `emed_sql/migrations/applied/2026-08-05_webhook_cooldown.sql`. **UI**: distinct "Paused — auto-retry <time>" (orange) vs "Disabled — 30d+ failures" (red) vs plain "Disabled" (manual) badges. **Operator recovery**: editing/re-enabling/redeliver in the API-portal clears the cool-down state (fresh retry window). To send a missed backlog after an outage, `moct.script.tracking_added` (the events clinics care about) is NOT auto-caught-up when tracking is >7 days old (the proc's `fire_ok` recency gate) or for scripts created during the gap (silently baselined) — enqueue it explicitly from `emed_script_status_snapshot` (snapshot has tracking + no prior `tracking_added` delivery, scoped to the owner's clinic variants + a `--since` ship_date, paced). Pattern: `emed_app/scripts/backfill_helimeds_tracking_webhooks.js`.
