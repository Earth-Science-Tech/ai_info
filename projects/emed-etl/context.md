...
2. **Peaks Curative ETL** — WooCommerce + WPForms → `liberty_link_stage` → eMed API (`POST /api/public/moct-visit`). Driven by the `peaks_*_flow.py` flows. Order-status sync had a creation-window gotcha (fetched by date_created, so late `processing→completed` changes froze in eMed) — fixed with a `modified_after` pull + a backfill tool; see [woo-orders-status-sync.md](woo-orders-status-sync.md). Shipment tracking back to the Peaks WordPress site (AST Pro plugin) runs as a subflow of the Liberty `run_all_etl` orchestrator — see [ast-shipment-tracking.md](ast-shipment-tracking.md) for the pipeline, the silent-failure gotcha, and the manual backfill tool.
...
6. **RingCentral call recordings** — two flows: `flows/calls/calls_fetch_recordings.py` syncs
   call-log **metadata** → `dbo.emed_call_recording` (live in prod), and
   `flows/calls/calls_archive_recordings.py` copies the **audio** to Azure Blob before
   RingCentral's ~90-day purge (new 2026-07-25, not yet in prod — needs a storage account).
   Audio deliberately does not go in SQL. RingSense AI insights were removed 2026-07-24 (not
   licensed). Permission model, the 90-day cliff, PHI rules + gotchas in
   [ringcentral-calls.md](ringcentral-calls.md).
