...
prs:
  ...
  - "emed_app#840 (feat->main)"
  - emed_app#841 (feat->main, store-order statuses follow the store)
tags: ["1.0.241", "1.0.313", "1.0.314", "1.0.329", "1.0.330", "1.0.345", "1.0.346", "1.0.348", "1.0.354", "1.0.355"]
created: 2026-08-30
updated: 2026-09-16
...
## Status & history
- 2026-09-16 — Incident + fix + data repair (nicholas-cardell), tag **1.0.354** (PR emed_app#840): **a "Second person on this order" transfer moved the CUSTOMER'S OWN intake form** to the spouse's new visit (the dialog listed forms by name and date only, nothing said who filled them), "Wrong person" on her visit then voided it under her, his visit fell back to Missing Forms, a Re-process re-prescribed the female product onto him and he was texted a re-fill link. Reconstructed from the history/notes/submission/answer rows and verified by three independent reviewers. **Guardrails:** every form in the transfer dialog now says who filled it (name, sex, DOB, how it differs from the patient) and the patient's own form or a nameless form cannot move without an explicit confirmation; creating a second person with the customer's own phone, email or DOB is refused; split visits are prescribed and reviewed on their own lines only (`scope_lines_for_split`); a review hold judged on the wrong form no longer silences the rule; **Wrong person offers "Return it to <other patient>" first and voided forms get a Restore action** (`GET restore-targets` / `POST restore-submission`, files + answers travel, both holds re-evaluated). **Repair applied to prod after the deploy** (`scripts/oneoff/repair_order_113432_2026-09-16.js --apply`): form + files back on the customer, duplicate Rx retired, moved draft follows the spouse, stale link cancelled, spouse's borrowed contact cleared, both visits in Missing Forms — staff still owe Re-process on his visit and Send link to HER OWN contact on hers. CLAUDE.md item 69.
- 2026-09-16 — Store-order statuses follow the store (nicholas-cardell), tag **1.0.355** (PR emed_app#841): the staged `pn_woo_orders` / legacy `woo_orders` row was written once at first ingest and never refreshed (the outbox never re-runs a done row; the webhook dropped shipped/refunded/cancelled up front; legacy ids were skipped; the old site's ETL stopped at go-live) — 413 of 433 PeakNow orders read "processing". One function, `wc_ingest.refresh_staged_statuses` (status + date_modified only, never inserts, never touches a visit), called from the webhook receiver (non-actionable statuses, legacy ids, already-ingested orders), from every 15-min reconciler pass (the backstop for dropped / out-of-order deliveries; heartbeat note carries `statuses refreshed N`) and from the one-off `scripts/oneoff/resync_wc_order_statuses_2026-09-16.js`. **Catch-up applied to prod after the deploy:** 376 rows by the one-off + 143 the reconciler's boot pass had already refreshed → PN now shipped 252 / processing 128 / completed 43 / refunded 8 / cancelled 4; legacy open since Aug 1 fell 260 → 122 processing. The 16 legacy-range store orders eMed does not hold are $1 test checkouts from May–June (bulk-completed on go-live day) — nothing to backfill. Also this day: legacy order #85947 backfilled into `woo_orders` (id 41915). CLAUDE.md item 86.
- 2026-09-15 — Feature + data (nicholas-cardell), tag **1.0.348** (PR emed_app#816; **1.0.346** PR #811 = every staff intake action became a dialog): **"Accept questionnaire as intake" now counts a file under the visit’s Attachments as evidence and the dialog uploads the emailed PDF / screenshot in the same step** (saved as an Attachment first; description only when nothing can be attached). **One-off applied to prod:** the 86 legacy WPForms consultation entries submitted on peaknow.com after the last sync (Sep 3–15) imported into `wpforms_entries` under the OLD form ids (`wpforms_entry_id` = 20,000,000 + peaknow id), 20 auto-linked to 12 waiting visits (`scripts/oneoff/export_peaknow_legacy_wpforms_2026-09-15.py` + `import_legacy_wpforms_2026-09-15.js`). **Order reconciliation** (old site vs peaknow.com vs eMed): exactly one legacy order missing (#85947 — `backfill_legacy_order_85947_2026-09-15.js`, dry-run verified, awaiting Nick); `woo_orders` frozen at the 9/3 freeze (220 eMed-open legacy orders already closed on the store); `pn_woo_orders` status never refreshed after first ingest (queue dedups on the order id) — 241 shipped orders still read processing. CLAUDE.md item 69.
